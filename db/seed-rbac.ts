...
const defaultPermissions = [
  ...
  { name: 'View Permissions', slug: 'permissions.read', resource: 'permissions', action: 'read' },
  ...
  { name: 'Create Tasks', slug: 'tasks.create', resource: 'tasks', action: 'create' },
  { name: 'Update Tasks', slug: 'tasks.update', resource: 'tasks', action: 'update' },
  { name: 'Delete Tasks', slug: 'tasks.delete', resource: 'tasks', action: 'delete' },
  { name: 'Assign Tasks', slug: 'tasks.assign', resource: 'tasks', action: 'assign' },

  // Project Management
  { name: 'View Projects', slug: 'projects.read', resource: 'projects', action: 'read' },
  { name: 'Create Projects', slug: 'projects.create', resource: 'projects', action: 'create' },
  { name: 'Update Projects', slug: 'projects.update', resource: 'projects', action: 'update' },
  { name: 'Delete Projects', slug: 'projects.delete', resource: 'projects', action: 'delete' },

  // Settings
  { name: 'Manage Settings', slug: 'settings.manage', resource: 'settings', action: 'manage' },
  { name: 'Manage Security', slug: 'settings.security', resource: 'settings', action: 'security' },
...
async function seed() {
  ...
  try {
    ...
    const [adminRole] = await db
      ...
      .values({
        name: 'Administrator',
        description: 'Administrative access to manage users, roles, and content',
        isSystem: true,
      })
      .onConflictDoNothing()
      .returning()
    ...
    const [userRole] = await db
      ...
      .values({
        name: 'User',
        description: 'Basic user access with read and self-management permissions',
        isSystem: true,
      })
      .onConflictDoNothing()
      .returning()
    ...
    console.log('\n📋 Summary:')
    console.log('  - 4 system roles created: Super Admin, Administrator, User, Viewer')
    console.log(`  - ${allPermissions.length} permissions created`)
    console.log('\n💡 Next steps:')
    console.log('  1. Assign roles to users using the user management interface')
